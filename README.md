...
5つの強力な MCP サーバーを統合済み。

- **serena**: LSPベースのセマンティックコード検索・編集
- **mcp-ripgrep**: 高速コード検索（ripgrep）
- **ts-morph-refactor**: TypeScript/JavaScript リファクタリング（シンボルリネーム、ファイル移動）
- **refactor**: 正規表現ベースのコード変換
- **spec-workflow**: 仕様ベース開発サポート
...
- **大規模リファクタリング**: serena のシンボルリネーム、参照検索
- **コード検索**: mcp-ripgrep で高速検索
- **TS/JS リファクタリング**: ts-morph-refactor でファイル移動時の import 自動更新
