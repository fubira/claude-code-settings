...
- **コード内にローカルパス・サーバーパスを絶対に書かない**。`/home/user/...` や `192.168.x.x` 等のハードコードは禁止
- 環境固有の値（ホスト名、ディレクトリパス、IPアドレス等）は環境変数または設定ファイルで外部化する
- スクリプト内のパス解決は `$(dirname "$0")/..`、`import.meta.dirname`、`Path(__file__)` 等の相対解決を使う
- テストのフィクスチャにも実在するパスを使わない（`/tmp/test-dir` 等の汎用パスを使う）
